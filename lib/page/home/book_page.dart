...
class _HomePage extends State<BookPage> with TickerProviderStateMixin {
  ...
  ///默认选中tab文本大小
  final _mUpperBound = DReaderConstant.textSize_20;
  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      ...
      if (_mTapScroll && _mScrollWidth == _scrollController.offset ||
          _mScrollWidth == _scrollController.offset) {
        _refreshTapScroll();
        _mScrollDirect = ScrollDirect.AIDL;
        print("停止滚动$_mScrollDirect $_mTapScroll");
      }
    });
  }
...
//      print("$mMilliseconds - $mUpperBound - $_mScrollDirect");
  ...
  void _onTapTab(index, mRootViewWidth, mChildrenWidth) {
    ...
    if (mNeedScrollOffset > 0) {
      ...
      if (index == 0) {
        _mScrollWidth = 0;
      } else if (index != _mTabIndex && index == _title.length - 1 ||
          index == _title.length - 1 && _mScrollWidth != mNeedScrollOffset) {
        ///点击最后个tab 滚动距离为超出屏幕的距离
        _mScrollWidth = mNeedScrollOffset;
      } else {
        ///中间滚动的距离为 mNeedScrollOffset/title.length*index 的距离
        _mScrollWidth = mNeedScrollOffset / _title.length * (index + 0.6);
      }
    ...
    return Column(
      ...
      children: <Widget>[
        DReaderTabInheritedWidget(
          size: _mTabTxtSize[_tabIndex(item)],
          defaultSize: _mDefaultTabTxtSize[_tabIndex(item)],
          child: DReaderTab(
            text: item,
